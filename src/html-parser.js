	...
	// 元素起始
	{
		...
		handler(str, context) {
			...
			str = str.replace(/^[^\s\/]+/, (match) => {
				...
				closed = true;
				return '';
			}).replace(/([^\s=]+)=(['"])(.*?)\2/g, (match, attrName, quot, attrValue) => {
				// 解析属性名、属性值（「a="b"」的模式）
...
exports.parse = function(html, context) {
	...
	while (html) {
		...
		theSymbol = theSymbol || textSymbol;
		...
		// 截取开始符和结束符之间的内容
		content = html.substring(theSymbol.startsWith.length, index);

		// 处理内容
		theSymbol.handler(content, context);

		// 截掉已处理内容
		html = html.substring(
			index + (theSymbol === textSymbol ? 0 : theSymbol.endsWith.length)
		);
	}
